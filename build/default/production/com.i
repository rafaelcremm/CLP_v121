...
switch(cmd){
case CMD_STATUS:
value = (ulong)oscCfg.status.INV.set.value;
break;
case CMD_FREQ_CFG:
...
if(serial.value > CMD_CTRL_LAST){

Cmd_t cmd = (serial.value - 32);
...
oscCfg.status.INV.set.value = (uint)serial.value;

break;
case CMD_REQUEST:
sendCMD((Cmd_t)serial.value);
break;
case CMD_FREQ_CFG:
